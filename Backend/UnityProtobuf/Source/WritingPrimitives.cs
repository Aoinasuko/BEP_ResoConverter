...
#region Writing of values (not including tags)
        ...
        public static unsafe void WriteFloat(ref Span<byte> buffer, ref WriterInternalState state, float value)
        {
            ...
            if (buffer.Length - state.position >= length)
            {
                ...
                MemoryMarshal.Write(floatSpan, ref value);
        ...
        {
            // BEP Unity build: retain the validated ASCII path using bounds-checked spans,
            // removing the optional System.Runtime.CompilerServices.Unsafe dependency.
            for (int i = 0; i < length; i++) buffer[state.position + i] = (byte)value[i];
            state.position += length;
        }
