...
public partial class RootConverter : IDisposable
{
    ...
    private readonly ConverterSettings _options;
    ...
    public RootConverter(EngineController engine, f::World world, StatusStream stream, ConverterSettings? options = null)
    {
        _options = options ?? new ConverterSettings();
        InitComponentTypes();
    ...
    private async Task<ByteString> _ConvertSync(p.ExportRoot exportRoot)
    {
        ...
        // Only renderers mapped from input components, excluding imported helper gadgets.
        var sourceRenderers = _objects.Values.OfType<f.MeshRenderer>().ToHashSet();
        var visibleSourceRenderers = sourceRenderers.Where(r => r.Enabled && r.Slot.IsActive).ToArray();
        ...
        // AvatarCreator and imported helper gadgets may attach their own protections.
        // Apply the selected save policy once, consistently, to the exported object root.
        foreach (var oldProtection in _root.GetComponentsInChildren<SimpleAvatarProtection>().ToArray())
        {
            // These protections were created in this isolated conversion world by AvatarCreator.
            // Use the same permission check and removal flag as its native inspector action.
            if (!oldProtection.CanUse(_world.LocalUser))
                throw new InvalidOperationException("A helper asset's save protection cannot be changed by the local conversion user.");
            var allowRemoval = typeof(SimpleAvatarProtection).GetField("_allowRemoval", BindingFlags.Instance | BindingFlags.NonPublic)
                ?? throw new MissingFieldException("Resonite SimpleAvatarProtection removal API has changed.");
            allowRemoval.SetValue(oldProtection, true);
            oldProtection.Destroy();
        }
        if (_options.lockSaving)
        {
            var protection = _root.AttachComponent<SimpleAvatarProtection>();
            protection.ReassignUserOnPackageImport.Value = true;
        }
        ...
        {
            var bounds = await BepBounds.Compute(visibleSourceRenderers, _root.Parent);
            await new f.ToWorld();
            var height = bounds.Size.y;
            if (float.IsFinite(height) && height > 0.01f && _options.standardHeight > 0.1f)
                _root.LocalScale *= _options.standardHeight / height;
        }
        ...
        var exportedRenderers = _root.AddSlot("[BEP] Exported Renderers");
        foreach (var renderer in sourceRenderers)
            exportedRenderers.AttachComponent<f.ReferenceField<f.MeshRenderer>>().Reference.Target = renderer;
        var visibleExportedRenderers = _root.AddSlot("[BEP] Visible Exported Renderers");
        foreach (var renderer in visibleSourceRenderers)
            visibleExportedRenderers.AttachComponent<f.ReferenceField<f.MeshRenderer>>().Reference.Target = renderer;
