...
public partial class RootConverter
{
    ...
    {
        if (!float.IsFinite(collider.Radius) || collider.Radius <= 0) return null;
        float height;
        switch (collider.Type)
        {
            ...
            case p.ColliderType.Capsule:
                height = Math.Max(0, collider.Height - 2 * collider.Radius);
                break;
            default:
                // unsupported
                return null;
        ...
        int colliderCount = Math.Clamp(1 + (int)Math.Ceiling(height / collider.Radius), 1, 64);

        float interval = colliderCount > 1 ? height / (colliderCount - 1) : 0;
        float3 posOffset = collider.PositionOffset.Vec3();
        float3 heightOffset = -(height / 2f * float3.Up);
        
        Defer(PHASE_BUILD_COLLIDERS, "Build DynBone colliders", async () =>
        {
            var root = Object<f.Slot>(collider.TargetTransform) ?? parent;
    ...
    {
        if (bone.Bones.Count == 0) return null;
        ...
        Defer(PHASE_RESOLVE_REFERENCES, "Configure dynamic bones", async () =>
        {
            ...
            if (bone.VrcParameters)
            {
                ...
                var pull = Math.Clamp(bone.VrcPull, 0, 1);
                var spring = Math.Clamp(bone.VrcSpring, 0, 1);
                db.Elasticity.Value = 1000f * pull;
                db.Damping.Value = 60f * (1f - spring);
                db.Stiffness.Value = Math.Clamp(bone.VrcStiffness, 0, 1);
                db.Inertia.Value = 1f - Math.Clamp(bone.VrcImmobile, 0, 1);
                db.Gravity.Value = new float3(0, -9.81f * bone.VrcGravity, 0);
                db.UseUserGravityDirection.Value = false;
                db.GravitySpace.UseParentSpace.Value = false;
                var original = boneChild.AddSlot("BEP Source PhysBone (reference settings)").AttachComponent<ValueField<string>>();
                original.Value.Value = Google.Protobuf.JsonFormatter.Default.Format(bone);
            }
            else GenerateTemplateControls(db, templateName);
        });
