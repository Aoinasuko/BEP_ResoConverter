...
public partial class RootConverter
{
    ...
    private async Task<f.IWorldElement?> CreateMaterial(p::Asset asset, p::Material material)
    {
        ...
        {
            case p.MaterialCategory.Pbr:
                mat = CreateBepPbrMaterial(matSlot, material);
                break;
            case p.MaterialCategory.Unlit:
                mat = CreateBepUnlitMaterial(matSlot, material);
                break;
        ...
        Defer(PHASE_AVATAR_SETUP + 1, "Configure AFK material", () =>
        {
            ...
            foreach (var simpleAwayIndicator in _root.GetComponentsInChildren<f.SimpleAwayIndicator>())
            {
                if (simpleAwayIndicator.Slot.IsChildOf(_coreSys!))
                {
                    ...
                    if ((simpleAwayIndicator.AwayMaterial.Target as Component)?.Slot == _root)
                    {
    ...
    private async Task<f.IAssetProvider<Material>> CreateXSToonMaterial(string name, f.Slot holder, p.Material src)
    {
        ...
        Defer(PHASE_RESOLVE_REFERENCES, "Setting material texture references", () =>
        {
            ...
            if (src.ShadowRamp != null)
            {
                ...
                if (Asset<f.IAssetProvider<f.Texture2D>>(src.ShadowRamp) is f.StaticTexture2D ramp)
                    ramp.WrapMode = Renderite.Shared.TextureWrapMode.Clamp;
            }
        ...
        mat.ZWrite.Value = src.HasZWrite ? (src.ZWrite ? ZWrite.On : ZWrite.Off) : ZWrite.On;
        ...
        if (src.MatcapColor != null) mat.MatcapTint.Value = src.MatcapColor.ColorX();
        if (src.RimColor != null) mat.RimColor.Value = src.RimColor.ColorX();
        if (src.HasRimIntensity) mat.RimIntensity.Value = src.RimIntensity;
        if (src.HasRimRange) mat.RimRange.Value = src.RimRange;
        if (src.HasRimThreshold) mat.RimThreshold.Value = src.RimThreshold;
        if (src.HasRimSharpness) mat.RimSharpness.Value = src.RimSharpness;
    ...
    private void BindExemplarValues(f.XiexeToonMaterial mat, f.XiexeToonMaterial xsExemplar, p.Material src)
    {
        ...
        if (src.RimColor == null) BindField(mat.RimColor);
        BindField(mat.RimAlbedoTint);
        if (!src.HasRimIntensity) BindField(mat.RimIntensity);
        if (!src.HasRimRange) BindField(mat.RimRange);
        if (!src.HasRimThreshold) BindField(mat.RimThreshold);
        if (!src.HasRimSharpness) BindField(mat.RimSharpness);
