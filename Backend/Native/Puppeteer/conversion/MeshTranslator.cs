...
public static class MeshTranslator
{
    public static MeshX ToMeshX(this pm::Mesh rpcMesh)
    {
        ...
        for (int uv = 0; uv < Math.Min(4, rpcMesh.Uvs.Count); uv++)
        {
            var uvData = rpcMesh.Uvs[uv]!;
            for (int v = 0; v < uvData.Uvs.Count; v++)
            {
                // Unity stores UV channels as Vector4 even when the material only uses XY.
                var coordinate = uvData.Uvs[v];
                meshx.SetUV(v, uv, new float2(coordinate.X, coordinate.Y));
            }
        }
        ...
        if (meshx.HasBoneBindings)
        {
            ...
            for (int i = 0; i < rpcMesh.VertexBoneWeights.Count; i++)
            {
                ...
                var strongestWeights = vbw.BoneWeights.OrderByDescending(w => w.Weight).Take(4).ToList();
                var binding = meshx.RawBoneBindings[i];

                var weight0 = strongestWeights.GetOrNull(0);
                var weight1 = strongestWeights.GetOrNull(1);
                var weight2 = strongestWeights.GetOrNull(2);
                var weight3 = strongestWeights.GetOrNull(3);
