...
「両手の形に合わせて表情を変える」の下にある「コントローラー操作で手の形も切り替える」は初期値ONです。SteamVRのOculus/Meta Touchでは、トリガーへの接触と押し込み、グリップ、親指の接触から表情を判定し、手指の見た目も8種へ切り替えます。アイテムやツールを持っている手は、元の握り方を優先します。チェックを外すと手指の見た目を変更せず、現在の指の姿勢から表情を判定します。
