...
v0.3.3では、PCの移動・ジャンプで表情が変わる問題を抑制し、PCのハンドサインをShift＋数字キー操作に変更します。こちらも元のUnityシーンから再変換してください。
...
アバター出力では「両手の形に合わせて表情を変える」と「メニューから表情を変えられるようにする」を個別に有効にできます。左右のハンドサインはVRChat形式の8種と、条件を限定しない「指定なし（どの形でも）」（Any）から選びます。左右両方の条件が一致した行を使い、複数の行に一致した場合は上の行を優先します。

「両手の形に合わせて表情を変える」の下にある「VRのコントローラー操作で手の形も切り替える」は初期値ONです。VRのSteamVR Oculus/Meta Touchでは、トリガーへの接触と押し込み、グリップ、親指の接触から表情を判定し、手指の見た目も8種へ切り替えます。アイテムやツールを持っている手は、元の握り方を優先します。チェックを外すとVRでは手指の見た目を変更せず、現在の指の姿勢から表情を判定します。

PCでは**左Shift＋数字1～8で左手、右Shift＋数字1～8で右手**を操作します。キーボード上段の数字を使い、押している間だけ有効で、離すと解除します。1から順にIdle・Fist・Open・Point・Victory・RockNRoll・HandGun・ThumbsUpです。未操作の手は表情の条件判定ではIdleになり、どちらの手もキー操作していない間は、移動で指が変化してもハンドサイン表情を発動しません。このPC操作はVRのコントローラーチェックをOFFにしても使えます。

Viveなどの他のコントローラーは、現在の指の姿勢から判定する方式を使います。**Viveのトラックパッド操作を8種のハンドサインへ割り当てる機能は未対応です。** TouchもVRChat風の操作を目指した対応で、VRChatのバインド設定をそのまま移す機能ではありません。機器・設定によって認識が異なる場合があります。
...
メニュー項目を1件以上登録すると、Resonite上の「表情」メニューに「初期状態」を自動で追加します。メニュー表情はハンドサインより優先して固定され、両機能を使う場合は「ハンドサインに戻す」で手による切り替えを再開できます。この固定は顔の表情だけに適用し、コントローラーによる手形の切り替えは続きます。初期状態は変換時のモデルの値を基準にし、表情が指定していないShapeの瞬き・口パクは維持します。3Dモデル・アイテム出力ではこれらの設定を無効化し、表情操作用の仕組みを付けません。
