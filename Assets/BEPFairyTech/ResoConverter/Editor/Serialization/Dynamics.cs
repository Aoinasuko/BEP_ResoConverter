...
namespace BEPFairyTech.ResoConverter
{
    ...
    internal partial class AvatarSerializer
    {
        ...
        private void AddPhysBoneEndpoints(GameObject root)
        {
            ...
            {
                if (component == null || component.GetType().FullName != "VRC.SDK3.Dynamics.PhysBone.Components.VRCPhysBone") continue;
        ...
        private IMessage TranslateDynamicBone(Component component)
        {
            ...
            foreach (var item in nodes)
            {
                ...
                message.Bones.Add(new p.DynamicBoneNode
                {
            ...
            if (OptionalComponent.Get(component, "limitType")?.ToString() != "None")
                Warnings.Add(component.name + ": PhysBone の角度制限は近似変換の対象外です。元設定を変換レポートに保存します。");
        ...
        private static float Sample(Component component, string name, float depth, float fallback)
        {
            float value = OptionalComponent.Get(component, name, fallback);
            var curve = OptionalComponent.Get<AnimationCurve>(component, name + "Curve");
            return value * (curve != null && curve.length > 0 ? curve.Evaluate(depth) : 1f);
        }
