...
| コントローラー操作で手の形も切り替える | 初期値ON。ハンドサイン表情を有効にした場合に表示。SteamVRのOculus/Meta Touch入力で表情と手形を切り替え |
...
### コントローラー操作で手の見た目も切り替える

「両手の形に合わせて表情を変える」を有効にすると、その下に「コントローラー操作で手の形も切り替える」が表示されます。このチェックの初期値はONです。ハンドサイン表情自体の初期値はOFFのため、ツールを追加しただけで手の動きが変わることはありません。
